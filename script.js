...
const interns = [
    { name: "Alice Johnson", interest: ["Marketing", "Finance"] },
    { name: "Bob Williams", interest: ["Technology", "Engineering"] },
...
function match() {
    ...
    for (let i = 0; i < entrepreneurs.length; i++) {
        ...
        matchedPairs.push({ entrepreneur, intern });
    }
...
function displayMatches(matches) {
    ...
    matches.forEach((match, index) => {
        ...
        matchElement.innerHTML = `
            <h2>Match ${index + 1}</h2>
            <p><strong>Entrepreneur:</strong> ${match.entrepreneur.name}</p>
            <p><strong>Expertise:</strong> ${match.entrepreneur.expertise.join(", ")}</p>
